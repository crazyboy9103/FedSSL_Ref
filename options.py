...
def args_parser():
    ...
    # Experimental setup
    parser.add_argument("--exp",              type=str,       default="FLSL",      choices=["simclr", "simsiam", "centralized", "fedbn", "FLSL"])
    ...
    parser.add_argument("--agg",             type=str,     default="fedavg", choices=["fedavg", "fedprox", "fedmatch", "FedSSL"])
    ...
    if args.exp == "FLSL":
        ...
        get_centralized_opts(args)
    
    elif args.exp == "simclr":
        get_simclr_opts(args)

    elif args.exp == "simsiam":
        get_simsiam_opts(args)

    # if args.agg == "FedSSL":
    #     get_FedSSL_opts(args)

    return args

def get_centralized_opts(args):
    if args.iid == True:
...
def get_FedSSL_opts(args):
    ...
    args.server_epochs = 5
    args.server_num_items = 300 # per class!
    args.finetune = True
    # freeze backbone
    args.freeze = False

def get_simclr_opts(args):
    if args.iid == True:
        args.alpha = 100000
    else:
        args.alpha = 0.5
